import { imgUrl } from "./imgUrl.js";

export const Workshops = ({ workshop, anchorId }) => {
  return (
    <div
      id={anchorId}
      className="flex-grow-0 flex flex-col items-start w-[95%] custom-440:w-[70%] text-sm relative px-0 custom-588:px-[1rem] py-[0.1rem] custom-588:py-[2rem] custom-590:flex-col custom-590:w-[50%] custom-1120:w-[50%] sm:w-[100%] sm:flex-row custom-1150:w-[50%] mx-auto sm:mx-0 gap-y-2 sm:gap-y-0 scroll-mt-24"
    >
      {/* Kuva näytetään vain jos se on olemassa */}
      {workshop.fields.artwork && (
        <div className="sm:pr-[2rem] overflow-hidden">
          <img
            src={imgUrl(
              workshop.fields.artwork.fields.file.url,
              "?fit=thumb&f=center&w=400&h=400&fm=webp"
            )}
            alt={`${workshop.fields.workshopName} artwork`}
            className="aspect-square w-full pr-0 h-auto custom-588:w-[20rem] min-h-[18rem] object-cover rounded-lg"
            loading="lazy"
          />
          {/* Kuvaajakrediitti kuvan alla */}
          {workshop.fields.photoCredit && (
            <div className="text-xs pt-[0.3rem] font-light">
              {workshop.fields.photoCredit}
            </div>
          )}
        </div>
      )}

      <div className="flex flex-col justify-between w-[100%] sm:w-[60%] custom-1150:w-[60%] custom-1120:w-[60%]">
        <div className="flex flex-col justify-between gap-[0.15rem]">
          <div className="flex flex-col justify-between custom-588:pt-0">
            <h3 className="text-2xl pt-[0.2rem] sm:py-[0.20rem] font-semibold font-serif">
              {workshop.fields.workshopName}
            </h3>

            {/* one instagram handle */}
            {workshop.fields.handle && !workshop.fields.handle2 && (
              <h4 className="text-base sm:py-[0.25rem] font-light font-serif hover:underline">
                <a
                  href={`https://www.instagram.com/${workshop.fields.handle}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  @{workshop.fields.handle}
                </a>
              </h4>
            )}

            {/* two instagram handles */}
            {workshop.fields.handle && workshop.fields.handle2 && (
              <h4 className="text-base sm:py-[0.25rem] font-light font-serif">
                <a
                  href={`https://www.instagram.com/${workshop.fields.handle}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:underline"
                >
                  @{workshop.fields.handle}
                </a>
                &nbsp;
                <a
                  href={`https://www.instagram.com/${workshop.fields.handle2}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:underline"
                >
                  @{workshop.fields.handle2}
                </a>
              </h4>
            )}
          </div>

          {/* Ei lowercase-muunnosta: aika-kentässä on paikannimi (Wanha Labra,
              Sininen Huone, Puisto), jonka iso alkukirjain pitää säilyä */}
          <div className="flex gap-[0.5rem] pt-[1rem] text-[1rem] font-semibold">
            <div>{workshop.fields.day}</div>
            <div>{workshop.fields.date}</div>
            <div>{workshop.fields.time}</div>
          </div>

          {workshop.fields.workshopLanguage && (
            <div className="flex flex-row">
              <div className="text-sm pt-[0.1rem] pr-[0.5rem] font-semibold">
                {workshop.fields.workshopLanguageTitle}
              </div>

              <div className="text-sm pt-[0.1rem] font-light">
                {workshop.fields.workshopLanguage}
              </div>
            </div>
          )}

          <p className="text-base min-h-[6.5rem] py-[0.2rem] font-light overflow-hidden whitespace-pre-line">
            {workshop.fields.description}
          </p>

          {/* Ilmoittautumislinkki (Fienta) */}
          {workshop.fields.fientaUrl && (
            <div className="pt-[0.5rem] pb-[0.5rem]">
              <a
                href={workshop.fields.fientaUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm font-semibold underline hover:no-underline"
              >
                {workshop.fields.fientaLabel}
              </a>
              {workshop.fields.fientaNote && (
                <div className="text-xs pt-[0.2rem] font-light">
                  {workshop.fields.fientaNote}
                </div>
              )}
            </div>
          )}
        </div>
      </div>
    </div>
  );
};
